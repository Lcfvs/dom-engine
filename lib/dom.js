...
function replaceAll (text, [match, value]) {
  return text.split(match).join(value ?? '')
}
...
function dom ({ DOMParser, NodeFilter, trustedTypes }, source) {
  let [, doctype, contents] = source.match(parts)
  const parser = new DOMParser()
  const fragment = parse(parser, doctype, contents, trustedTypes)
  ...
  return {
    doctype,
    node: tree(parse(parser, doctype, contents, trustedTypes))
  }
}

function filler (template, node) {
  const { [symbols.window]: window } = template
  const { Node } = windows.get(window)
  const { nodeType } = node
...
function fill (template) {
  ...
  if (node.nodeType === windows.get(window).Node.TEXT_NODE) {
...
function html (contents, factory) {
  if (factory && factory.createPolicy && !policies.has(factory)) {
    policies.set(factory, factory.createPolicy('dom-engine', policy))
  }
...
function parse (parser, doctype, contents, trustedTypes) {
  if (doctype.length) {
    const source = html(`${doctype}${contents}`, trustedTypes)

    return parser.parseFromString(source, 'text/html').documentElement
  } else {
    return parser
      .parseFromString(html('', trustedTypes), 'text/html')
      .createRange()
      .createContextualFragment(html(contents, trustedTypes))
  }
}

function replace (template) {
  const { [symbols.node]: node, [symbols.window]: window } = template
  const { document } = windows.get(window)
...
export function serialize (template) {
  ...
  const { XMLSerializer, trustedTypes } = windows.get(window)

  const source = new XMLSerializer()
    .serializeToString(render(template))

  return html(`${doctype}${source}`, trustedTypes)
}
